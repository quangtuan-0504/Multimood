...
def process_jsonl(file_path):
    ...
    Filters out responses containing AI refusal phrases.
    """
    ...
    # AI refusal phrases to check for
    ai_refusal_phrases = ["an ai language model", "can't assist with that"]
    ...
    with open(file_path, 'r', encoding='utf-8') as file:
        for line in file:
            ...
            # Extract prediction data first to check for refusal phrases
            predict_text = data.get('predict', '')
            predict_therapist_response_match = re.search(therapist_response_pattern, predict_text)
            
            # Skip this entry if the therapist's response contains refusal phrases
            if predict_therapist_response_match:
                response_text = predict_therapist_response_match.group(1).strip().lower()
                if any(phrase in response_text for phrase in ai_refusal_phrases):
                    # Skip this entry since it contains a refusal phrase
                    continue

            # If we're here, it means the prediction doesn't contain refusal phrases
            # Extract label data
            ...
            # Extract complete prediction data
            ...
            # We already extracted the response match above
            predict_therapist_response = predict_therapist_response_match
            ...
            predict_therapist_responses.append(response_text if predict_therapist_response else None)
...
def main():
    ...
    print(f"Filtered out responses containing AI refusal phrases. Remaining entries: {len(df)}")
