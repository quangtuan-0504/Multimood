...
def tokenizer_multimodal_token(prompt : str , tokenizer, multimodal_token : str = '<video>', return_tensors=None):
    ...
    if multimodal_token_index is None:
        ...
        input_ids = []
        for i in range(1, 2 * len(prompt_chunks)): # giả sử trong chuỗi prompt(prompt là 1 chuỗi) chỉ có 1 token <video> : text1 <video> text2 , duyệt chuỗi range(1,4)
            if i % 2 == 1:
