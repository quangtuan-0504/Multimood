...
def tokenizer_multimodal_token(prompt, tokenizer, modal_index_map, return_tensors=None):
    """
    Tokenize text and replace multimodal tags with corresponding indices.

    Args:
        prompt (str): Text prompt (w/ multimodal tag), e.g., '<video>\nDescribe the video.'
        tokenizer (transformers.PreTrainedTokenizer): Tokenizer object.
        modal_index_map (dict): Dictionary mapping multimodal tags to their token indices.
        return_tensors (str, optional): If 'pt', returns a PyTorch tensor.
    
    Returns:
        list[int] or torch.Tensor: Tokenized input with multimodal token indices replaced.
    """
    # Tokenize normally
    input_ids = []
    current_chunk = ""
    ...
    while i < len(prompt):
        ...
        for modal_token in modal_index_map.keys():
            if prompt[i:].startswith(modal_token):
                found_modal = modal_token
                break
        
        if found_modal:
            # Process preceding text chunk
            if current_chunk:
                input_ids.extend(tokenizer(current_chunk, add_special_tokens=False).input_ids)
                current_chunk = ""

            # Add multimodal token index
            input_ids.append(modal_index_map[found_modal])
            i += len(found_modal)  # Move index forward past the multimodal token
        else:
            current_chunk += prompt[i]
            i += 1

    # Process remaining text chunk if exists
    if current_chunk:
        input_ids.extend(tokenizer(current_chunk, add_special_tokens=False).input_ids)
