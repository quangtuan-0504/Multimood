...
def model_init(model_path=None, **kwargs):
    ...
    print("Model's name is used to run:",model_name)
...
def mm_infer(image_or_video, instruct, model, chat_history,  tokenizer, modal='video', **kwargs):
    ...
    if isinstance(instruct, str):
        ...
        # message[0]['content'] = modal_token + '\n' + message[0]['content'] # if in the prompt have <video> token then this here have two token <video> , so it error
        # so I remove one modal_token
    ...
    modal_ids = MODAL_INDEX_MAP.get(f'<{modal}>',None)#151647
    input_ids = tokenizer_multimodal_token(prompt, tokenizer, {modal_token: modal_ids}, return_tensors='pt').unsqueeze(0).long().cuda()
    ...
    input_ids = input_ids[:,-kwargs.get('max_prompt_tokens',4096):]
    attention_masks = attention_masks[:,-kwargs.get('max_prompt_tokens',4096):]
    chat_history_ids = chat_history["input_ids"][:,-kwargs.get('max_prompt_tokens',4096):] 
    ...
    with torch.inference_mode():
        output_ids = model.generate(
            ...
            images=tensor,
            history_input_ids=chat_history_ids, 
            do_sample=do_sample,
