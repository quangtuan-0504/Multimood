...
from .encoder import build_vision_tower, build_audio_tower, build_mamba_compressor
...
class Videollama2MetaModel:
    ...
    def __init__(self, config):
        ...
        if hasattr(config, "mamba_compressor"):
            self.mamba_compressor = build_mamba_compressor(config, delay_load=True)
    ...
    def initialize_audio_modules(self, model_args, fsdp=None):
        ...
        self.config.hidden_size_a = model_args.hidden_size
    ...
    def initialize_mamba_compressor_modules(self, model_args, fsdp=None):
        mamba_compressor_model = model_args.mamba_compressor
        self.mamba_compressor = MambaCompressor(
                mamba_path = mamba_compressor_model, 
                llm_input_size = model_args.hidden_size  ,
                tokenizer_len = model_args.tokenizer_len, 
                mem_token_id= model_args.mem_token_id, 
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            if labels is not None:
                ...
                input_length = cur_input_ids.shape[0]
                label_length = cur_labels.shape[0]

                # Determine the target length (maximum of the two)
                target_length = max(input_length, label_length)

                # Pad cur_input_ids if it's shorter
                if input_length < target_length:
                    ...
                    cur_input_ids = torch.cat((cur_input_ids, padding), dim=0)

                # Pad cur_labels if it's shorter
                if label_length < target_length:
                    ...
                    cur_labels = torch.cat((cur_labels, padding), dim=0)
            ...
            while mm_token_indices.numel() > 0:
                ...
                if labels is not None:
                    ...
                    if cur_input_ids[mm_token_start] == MODAL_INDEX_MAP['<history>']:
                        if have_hist:
                            cur_new_labels.append(torch.full((hist_features[cur_his_idx - 1].shape[0],), IGNORE_INDEX, device=labels.device, dtype=labels.dtype))
                    else:
