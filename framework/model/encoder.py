...
def build_audio_tower(audio_tower_cfg, delay_load: bool = False, **kwargs):
    audio_tower = getattr(audio_tower_cfg, 'mm_audio_tower', getattr(audio_tower_cfg, 'audio_tower', None))
    
    if not delay_load:
        try:
            audio_model_repo = audio_tower_cfg.pretrain_projectors_repo
            # Download the model checkpoint dynamically
            audio_checkpoint_path = hf_hub_download(repo_id=audio_model_repo, filename=audio_tower)
            
            # Load the checkpoint
            beats_checkpoint = torch.load(audio_checkpoint_path, map_location='cpu')
            ...
            beats.load_state_dict(filtered_checkpoint, strict=False)

        except Exception as e:
            raise ValueError(f"Error loading audio model from {audio_model_repo}/{audio_model_filename}: {e}")
