...
def build_mamba_compressor(config, delay_load=False, **kwargs):
    mamba_compressor_model = getattr(config, 'mamba_compressor', getattr(config, 'mamba_compressor', None))
    if not delay_load:
        print("Not implemented yet")
    else:
    
        mamba_compressor = MambaCompressor(
                mamba_path = mamba_compressor_model, 
                llm_input_size = config.hidden_size  ,
                tokenizer_len = config.tokenizer_len, 
                mem_token_id= config.mem_token_id, 
                torch_dtype = config.torch_dtype
        )
    return mamba_compressor
