...
from .smes_classification_llama import SMESModelLlamaConfig, SMESModelLlamaModel, SMESModelLlamaForClassification
...
VLLMs = {
    "SMES_llama": SMESModelLlamaForClassification,
...
VLLMConfigs = {
    "SMES_llama": SMESModelLlamaConfig,
...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto", device="cuda", use_flash_attn=False, **kwargs):
    ...
    if 'lora' in model_name.lower() or 'qlora' in model_name.lower():
        ...
        if model_type in ['videollama2', 'videollama2_mistral']:
            model = Videollama2MistralForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=True, config=config, **kwargs)
        elif model_type in ['SMES_llama']:
            model = SMESModelLlamaForClassification.from_pretrained(model_base, low_cpu_mem_usage=True, config=config, **kwargs)
        ...
        if model_type in ['videollama2', 'videollama2_mistral']:
            ...
            model = SMESModelLlamaForClassification.from_pretrained(model_path, low_cpu_mem_usage=True, config=config, **kwargs)
        elif model_type in ['videollama2_mixtral']:
