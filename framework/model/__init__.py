...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto", device="cuda", use_flash_attn=False, **kwargs):
    ...
    config = AutoConfig.from_pretrained(model_path)# hardcode by replace to 'DAMO-NLP-SG/VideoLLaMA2.1-7B-AV'
    # print(config)
    # return 0
    ...
    if 'lora' in model_name.lower() or 'qlora' in model_name.lower():
        ...
        # model_base = model_base if model_base is not None else config._name_or_path
        model_base = 'DAMO-NLP-SG/VideoLLaMA2.1-7B-AV'
        config_model_base = AutoConfig.from_pretrained(model_base , token = None)
        ...
        print("Base Model's Name , Not quantization:",model_base)
        ...
        # Use it if you saved tokenizer of finetuning model 
        tokenizer = AutoTokenizer.from_pretrained(model_path, use_fast=False, token=token)
        print("Special token be added:" , tokenizer.all_special_tokens)

        print('Loading VideoLLaMA from base model...')

        if 'vicuna' in model_base.lower():
            model = Videollama2LlamaForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=True, config=config_model_base, **kwargs)
        elif 'mistral' in model_base.lower():
            model = Videollama2MistralForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=True, config=config_model_base, **kwargs)
        elif model_type in ['videollama2', 'videollama2_mistral']:
            ...
            # model = Videollama2MistralForCausalLM.from_pretrained('DAMO-NLP-SG/VideoLLaMA2.1-7B-AV', low_cpu_mem_usage=True, config=config, **kwargs)
            model = Videollama2Qwen2ForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=True, config=config_model_base, **kwargs)
        ...
        elif model_type in ['videollama2_llama']:
            model = Videollama2LlamaForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, config=config, **kwargs)  
        else:
            model = Videollama2MistralForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, config=config, **kwargs)
    else:
